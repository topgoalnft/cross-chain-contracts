...
contract BridgeB is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // bridge token from B to A. step1: transfer token to bridge
    ...
    // bridge token from A to B. step2: withdraw token from bridge
    function withdrawToken(
        ...
        require(
            IMirrToken(req.toToken).mint(_msgSender(), req.amount),
            "Token transfer failed"
        );
